...
foreach ($file in Get-ChildItem -LiteralPath $Root -Filter '*.html') {
  ...
  foreach ($toggle in [regex]::Matches($html, '<button\b[^>]*\bid="navToggle"[^>]*>')) {
    ...
    if ($toggle.Value -notmatch '\baria-expanded="false"') { $failures.Add("$name nav toggle missing initial aria-expanded state") }
  }
  ...
  foreach ($img in [regex]::Matches($html, '<img\b[^>]*>')) {
    ...
    if ($img.Value -notmatch '\bwidth="\d+"') { $failures.Add("$name image missing width") }
    if ($img.Value -notmatch '\bheight="\d+"') { $failures.Add("$name image missing height") }
  }
}
